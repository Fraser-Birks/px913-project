...
for i in range(N_particles):
    ...
    vel_x = Vx[:,i][0:-1:250]
    vel_y = Vy[:,i][0:-1:250]
    acc_u = acc_x[:,i][0:-1:250]
    acc_v = acc_y[:,i][0:-1:250]
    # plot particle path
    axs[0,0].plot(part_x[:,i],part_y[:,i], label=f'position of particle {i+1}', alpha=0.8)
...
graphlines = []
for i in range(N_particles):
    path, = axs[0,1].plot(part_x[:,i][0], part_y[:,i][0], label='particle path')
    scatpoint, = axs[0,1].plot(part_x[:,i][0], part_y[:,i][0], marker=".", label='particle')
    graphlines.append(path)
    graphlines.append(scatpoint)
...
def update(t):
    ...
    for i in range(N_particles):
        graphlines[2*i].set_data(part_x[:,i][0:t], part_y[:,i][0:t])
        graphlines[(2*i)+1].set_data(part_x[:,i][t], part_y[:,i][t]) 
    return graphlines
